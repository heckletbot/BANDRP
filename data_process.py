...
class dataloader5fold(Data.Dataset):
...
def process5fold(drug_feature, mut_feature, exp_feature, methy_feature, pathway_feature, pair, cellline_id, drug_id):
    ...
    train_set = []
    test_set = []
    for i, (train_index, test_index) in enumerate(kf.split(pair)):
        train_set.append(Data.DataLoader(
            dataloader5fold(drug_data, expression, mutation, methylation, pathway, all_pairs, train_index),
            **params))
        test_set.append(
            Data.DataLoader(
                dataloader5fold(drug_data, expression, mutation, methylation, pathway, all_pairs, test_index),
                **params))
    return train_set, test_set


def process5fold2(drug_feature, mut_feature, exp_feature, methy_feature, pathway_feature, pair, cellline_id, drug_id):
    # cell_line drug ic50 pairs
    cellline_id.sort()
    drug_id.sort()
    cell_map = list(zip(cellline_id, list(range(len(cellline_id)))))
    ...
    train_set=Data.DataLoader(dataloader5fold(drug_data, expression, mutation, methylation, pathway, all_pairs, train_index),
            **params)
    test_set= Data.DataLoader(dataloader5fold(drug_data, expression, mutation, methylation, pathway, all_pairs, test_index),
            **params)
    val_set= Data.DataLoader(dataloader5fold(drug_data, expression, mutation, methylation, pathway, all_pairs, val_index),
            **params)
